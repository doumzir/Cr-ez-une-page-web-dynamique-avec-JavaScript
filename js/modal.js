...
window.addEventListener('click', function (event) {
    ...
    if (modal === event.target || closeModalLink === event.target) {
...
function addElementOnpage(idelement, srcImage, newTitle, newCat) {
    ...
    document.querySelector(".galleryEdit").innerHTML += `<figure class="category${newCat} show article article${idelement}" id="article${idelement}" >
        <img onclick="deletcomponant(${idelement})" alt="${newTitle}" id="trash${idelement}"class = "trash" src="assets/icons/trash.svg"</img>
					<img crossorigin="anonymous" src="${srcImage}" alt="${newTitle}">
...
document.getElementById('works').addEventListener('submit', function (e) {
    ...
    fetch('http://localhost:5678/api/works', {
        ...
        .then(() => {
            addElementOnpage(obj.id, obj.imageUrl, obj.title, obj.categoryId);
        })
        .then(() => {
            returnMenuModal();
        })
        .catch(err => alert("une erreur est survenue veuillez réessayer ultérieurement"));

})

function deleteDynamic(dynamicIdDelet) {
    let x = document.getElementsByClassName(`article${dynamicIdDelet}`);
    let i;
